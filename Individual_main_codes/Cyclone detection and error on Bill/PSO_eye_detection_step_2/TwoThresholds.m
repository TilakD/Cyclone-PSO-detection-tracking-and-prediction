...
%      k=k+1; 
 ...
 % Values to be compared with (as given by BF Algorithm)
 ReferenceValueLena = 0.8889;
 ReferenceValuePeppers = 0.8889;
 ReferenceValueBaboon = 0.8889;
 ReferenceValueCameraman = 0.8889;
 ReferenceValueAirplane = 0.888900;
 
 
  if imSelect==1
  fprintf('Objective Value (Should be > %0.4f ) = %0.4f ', ReferenceValueLena, funcVal);
 end
 
  if imSelect==2
    fprintf('Objective Value (Should be > %0.4f ) =%0.4f ', ReferenceValuePeppers, funcVal);
  end
 
   if imSelect==3
     fprintf('Objective Value (Should be > %0.4f ) = %0.4f  ', ReferenceValueBaboon, funcVal);
   end
 
    if imSelect==4
      fprintf('Objective Value (Should be > %0.4f ) = %0.4f ', ReferenceValueCameraman, funcVal);
    end
 
  if imSelect==5
   fprintf('Objective Value (Should be > %0.4f ) = %0.4f ', ReferenceValueAirplane, funcVal);
 end
 fprintf('\n');
